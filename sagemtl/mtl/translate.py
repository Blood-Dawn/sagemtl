...
class _Base:
    device: str
    def translate(self, text: str) -> str: raise NotImplementedError
...
class _CT2(_Base):
    def __init__(self, model: str, device: str):
        ...
        self.translator = ctranslate2.Translator(model, device="cuda" if (device=="cuda" or (device=="auto" and torch.cuda.is_available())) else "cpu")
        # assume sentencepiece model files named src.model and tgt.model beside model dir
        self.src_sp = spm.SentencePieceProcessor(model_file=str((__import__("pathlib").Path(model)/"src.model").resolve()))
        self.tgt_sp = spm.SentencePieceProcessor(model_file=str((__import__("pathlib").Path(model)/"tgt.model").resolve()))
